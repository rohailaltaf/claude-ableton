...
export function registerTools(server: McpServer): void {
  ...
  server.registerTool(
    ...
    {
      description:
        "List every scene (row) with its index, name, whether it's empty, and any " +
        "per-scene tempo / time-signature override (tempo & time_signature are null " +
        "unless the scene sets one — see set_scene_tempo / set_scene_time_signature). " +
    ...
    async () => {
      ...
      for (let k = 0; k < n; k++) {
        ...
        // Colorless scenes report None — the reply carries no value. Surface as null.
  ...
  server.registerTool(
    ...
    {
      description:
        "Set a per-scene tempo. When the scene is fired, Live jumps to this BPM and " +
        "holds it until another tempo-bearing scene fires — the way you build " +
        "tempo changes into a Session set. Pass enabled=false to clear the scene's " +
        "tempo override (the scene stops dictating tempo). Range 20-999. Read back " +
        "via list_scenes.",
      inputSchema: {
        scene_index: z.number().int().describe("zero-based scene index"),
        bpm: z.number().describe("scene tempo in BPM (20-999)"),
        enabled: z
          .boolean()
          .default(true)
          .describe("false clears the scene's tempo override instead of setting it"),
      },
    },
    async ({ scene_index, bpm, enabled }) => {
      ...
      const c = await getClient();
      // Writing a valid tempo only makes sense when enabling; when disabling we
      // just flip the override off (Live reports tempo as -1 while disabled).
  ...
  server.registerTool(
    ...
    {
      description:
        "Set a per-scene time signature. When the scene is fired, Live switches to " +
        "this meter — lets you change meter across a Session set scene by scene. " +
        "Common sigs: 4/4, 3/4, 6/8, 7/8, 5/4. Pass enabled=false to clear the " +
        "scene's time-signature override. numerator 1-99, denominator a power of 2 " +
        "(1,2,4,8,16,32,64). Read back via list_scenes.",
      inputSchema: {
        scene_index: z.number().int().describe("zero-based scene index"),
        numerator: z.number().int().describe("beats per bar (1-99)"),
        denominator: z.number().int().describe("beat unit, power of 2 (1,2,4,8,16,32,64)"),
        enabled: z
          .boolean()
          .default(true)
          .describe("false clears the scene's time-signature override instead of setting it"),
      },
    },
  ...
  server.registerTool(
    ...
    {
      description:
        "Set a scene's color. Provide either color (packed RGB 0xRRGGBB — Live snaps to " +
        "the nearest swatch) or color_index (0-69, Live's palette). Pass exactly one. " +
        "Read back via list_scenes (color_index is null on scenes that have never been " +
        "colored; once set it can't be returned to colorless via the API).",
      inputSchema: {
  ...
  server.registerTool(
    ...
    {
      description:
        "List the grooves currently in the Groove Pool — swing/feel templates that can be " +
        "applied to clips. Each: groove_index, name, base (the grid it operates on), and " +
        "its amounts — quantization (pull toward the grid), timing (how much groove swing), " +
        "random (timing randomization → humanize), velocity (velocity variation). Assign " +
        "one to a clip with set_clip_groove; tune the amounts with set_groove_params; the " +
        "global groove_amount (set_groove_amount) scales them all. NOTE: grooves can't be " +
        "loaded from disk via Live's API — this manages grooves already in the pool (from " +
        "loaded clips/packs, or dragged in via Live's browser).",
      inputSchema: {},
    },
    async () => {
      const c = await getClient();
  ...
  server.registerTool(
    ...
    {
      description:
        "Tune a groove in the Groove Pool (by groove_index from get_groove_pool). Editing " +
        "a groove changes the feel of EVERY clip using it. Amounts are percentages: " +
        "timing (how much of the groove's swing, ~0-130), quantization (snap toward the " +
        "grid first, 0-100), random (timing randomization for human feel, 0-100), velocity " +
        "(velocity variation, -100 to 100). Provide any subset.",
      inputSchema: {
        groove_index: z.number().int().describe("index from get_groove_pool"),
        timing: z.number().optional().describe("groove timing/swing amount (~0-130)"),
        quantization: z.number().optional().describe("quantize-toward-grid amount (0-100)"),
        random: z.number().optional().describe("timing randomization / humanize (0-100)"),
        velocity: z.number().optional().describe("velocity variation (-100 to 100)"),
      },
    },
    async ({ groove_index, timing, quantization, random, velocity }) => {
      if (timing === undefined && quantization === undefined && random === undefined && velocity === undefined) {
        throw new Error("provide at least one of timing, quantization, random, velocity");
      }
      const c = await getClient();
      const send = (prop: string, v: number) =>
        c.send("/live/song/groove_pool/set/groove", i(groove_index), prop, f(v));
      if (timing !== undefined) send("timing", timing);
      if (quantization !== undefined) send("quantization", quantization);
      if (random !== undefined) send("random", random);
  ...
  server.registerTool(
    ...
    {
      description:
        "Apply a Groove Pool groove to a clip (swing/feel). groove_index comes from " +
        "get_groove_pool. The groove's effect also depends on the global groove_amount " +
        "(set_groove_amount). NOTE: Live's API can ASSIGN a groove but cannot CLEAR one " +
        "(setting a clip's groove to none is rejected by Live) — to kill groove feel " +
        "globally set groove_amount to 0, or clear the clip's groove in Live's UI.",
      inputSchema: {
        track_index: z.number().int(),
        clip_slot: z.number().int().describe("must contain a clip"),
        groove_index: z.number().int().describe("index from get_groove_pool (>= 0)"),
      },
    },
    async ({ track_index, clip_slot, groove_index }) => {
      if (groove_index < 0) {
        throw new Error(
          "groove_index must be >= 0. Clearing a clip's groove isn't supported by Live's " +
            "API — use set_groove_amount(0) to disable groove globally, or clear it in Live.",
        );
      }
      const c = await getClient();
      ...
      if (groove_index >= poolCount) {
        throw new Error(
          `groove_index ${groove_index} out of range (pool has ${poolCount} groove(s)). ` +
            "See get_groove_pool.",
        );
      }
  ...
  server.registerTool(
    ...
    {
      description:
        "Stop currently-playing Session clips while the transport keeps running " +
        "(unlike stop_playing, which halts the transport). With no track_index, stops " +
        "every track's clips; with track_index, stops only that track's clips.",
      inputSchema: {
        track_index: z
          .number()
          .int()
          .optional()
          .describe("optional: stop only this track's clips; omit to stop all tracks"),
      },
    },
    async ({ track_index }) => {
      const c = await getClient();
  ...
  server.registerTool(
    ...
    {
      description:
        "Return playback control to the Arrangement (Live's 'Back to Arrangement' " +
        "button). Firing Session clips overrides the corresponding Arrangement tracks " +
        "— the button lights orange — and they stay overridden until this is called. " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Undo the last action in Live — identical to Cmd+Z in the app. Walks back " +
        "through Live's own undo history, so it covers changes made by these tools " +
        "(notes, devices, clips, mixer moves, scene edits) as well as the user's own " +
        "edits. Returns can_undo_more so you know whether further undo steps remain.",
      inputSchema: {},
    },
    async () => {
      const c = await getClient();
  ...
  server.registerTool(
    ...
    {
      description:
        "Redo the last undone action in Live — identical to Cmd+Shift+Z. Returns " +
        "can_redo_more so you know whether further redo steps remain.",
      inputSchema: {},
    },
    async () => {
      const c = await getClient();
  ...
  server.registerTool(
    ...
    {
      description:
        "Turn a clip's Legato launch on or off (a Session-clip launch-box setting). When " +
        "legato is on, launching this clip continues playback from the position of the " +
        "previously-playing clip on the track instead of restarting from the top — used " +
        "for seamless transitions between variations that stay in sync.",
      inputSchema: {
        track_index: z.number().int(),
        clip_slot: z.number().int().describe("must contain a clip"),
        legato: z.boolean(),
  ...
  server.registerTool(
    ...
    {
      description:
        "Set how much the velocity of the MIDI note that launches a clip scales the " +
        "clip's volume (the launch-box 'Velocity' amount). 0.0 = velocity has no effect " +
        "(default); 1.0 = full sensitivity, so a softer trigger plays the clip quieter. " +
        "Only audible when the clip is launched from a velocity-sensitive source.",
      inputSchema: {
        track_index: z.number().int(),
        clip_slot: z.number().int().describe("must contain a clip"),
        amount: z.number().describe("0.0-1.0"),
      },
    },
  ...
  server.registerTool(
    ...
    {
      description:
        "Read a single clip's live state and identity: type (midi/audio), is_playing, " +
        "is_triggered (queued, waiting on launch quantization), is_recording, muted, " +
        "playing_position (beats within the clip while playing), length_beats, and — " +
        "for audio clips — the file_path of the underlying sample (null for MIDI).",
      inputSchema: {
        track_index: z.number().int(),
        clip_slot: z.number().int().describe("must contain a clip"),
      },
    ...
    async ({ track_index, clip_slot }) => {
      ...
      if (!isMidi) {
        // file_path raises on MIDI clips (audio-only property), so only query
        // it once we know the clip is audio.
  ...
  server.registerTool(
    ...
    {
      description:
        "Collapse or expand a Group track (the triangle in the track header). Only " +
        "foldable tracks (groups) accept this — errors cleanly on regular tracks. " +
        "Note: groups can't be *created* via the API (UI-only, Cmd+G); this only " +
        "folds/unfolds existing ones.",
      inputSchema: {
        track_index: z.number().int(),
        folded: z.boolean().describe("true = collapsed, false = expanded"),
      },
    },
    async ({ track_index, folded }) => {
      ...
      if (!asBool(foldableReply[1])) {
        throw new Error(
          `track ${track_index} is not foldable (only Group tracks fold). ` +
            "Use list_tracks to find group tracks.",
        );
      }
  ...
  server.registerTool(
    ...
    {
      description:
        "Read a track's live state: output meters (output_meter_level mono summary plus " +
        "left/right, 0.0-1.0 momentary values — ~0.92 corresponds to 0 dB; useful for " +
        "'is this track making sound / too hot?') and which Session slot is playing " +
        "(playing_slot_index) or queued (fired_slot_index); -1 means none, -2 means " +
        "stop-button. Meters only move while audio is actually playing.",
      inputSchema: { track_index: z.number().int() },
    },
    async ({ track_index }) => {
      const c = await getClient();
  ...
  server.registerTool(
    ...
    {
      description:
        "Load a third-party plugin (VST/VST3/AU) or a saved rack onto the Main (master) " +
        "track — the master equivalent of load_browser_item, for mastering with your own " +
        "tools (e.g. FabFilter Pro-L, an Ozone rack). Pair with the matching list_* tool: " +
        "list_plugins → load_browser_item_on_master(node='plugins', path=...). For Live's " +
        "built-in effects (Glue Compressor, Limiter, EQ Eight) use " +
        "load_audio_effect_on_master instead. Devices append to the end of the chain.",
      inputSchema: {
        node: z
          .enum(["plugins", "user_library", "packs", "max_for_live"])
          .describe("which browser node the path is from (matches the list_* tool)"),
    ...
    async ({ node, path }) => {
      ...
      c.send("/live/master_track/load_browser_item", node, path);
      ...
      while (Date.now() < deadline) {
        ...
        try {
          ...
          if (count > before) {
            return jsonResult({ node, path, device_count: count, device_index: before });
          }
        } catch (e) {
      ...
      throw new Error(
        `Load timed out: ${node} item ${JSON.stringify(path)} did not appear on the Main ` +
          `track within ${LOAD_TIMEOUT_MS / 1000}s. Verify the path with list_${node}.`,
      );
    },
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "Delete a device from the Main (master) track by index (see get_master_devices). " +
        "Use to remove a wrongly-chosen mastering device. Destructive but Undo-able " +
        "(Cmd+Z). The regular delete_device can't reach the Main track.",
      inputSchema: {
        device_index: z.number().int().describe("index from get_master_devices"),
      },
    },
    async ({ device_index }) => {
      const c = await getClient();
