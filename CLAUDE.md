...
## Layout
- `src/*.ts` — the server. `tools.ts` (all 115 MCP tools), `osc.ts` (hand-rolled
...
- `scripts/` — `build.mjs` (esbuild), `vendor-remote-script.mjs`,
  `integration-test.mjs` (live 115-tool test), `list-tools.mjs`, `smoke-send.ts`.
...
- `node scripts/integration-test.mjs` drives all 115 tools (assertions + cleanup).
