...
## Layout
- `src/*.ts` — the server. `tools.ts` (all 139 MCP tools), `osc.ts` (hand-rolled
...
- `scripts/` — `build.mjs` (esbuild), `vendor-remote-script.mjs`,
  `integration-test.mjs` (live 139-tool test), `list-tools.mjs`, `smoke-send.ts`.
...
- `node scripts/integration-test.mjs` drives all 139 tools (assertions + cleanup).
