...
## Layout
- `src/*.ts` — the server. `tools.ts` (all 125 MCP tools), `osc.ts` (hand-rolled
...
- `scripts/` — `build.mjs` (esbuild), `vendor-remote-script.mjs`,
  `integration-test.mjs` (live 125-tool test), `list-tools.mjs`, `smoke-send.ts`.
...
- `node scripts/integration-test.mjs` drives all 125 tools (assertions + cleanup).
