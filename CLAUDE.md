...
## Layout
- `src/*.ts` — the server. `tools.ts` (all 132 MCP tools), `osc.ts` (hand-rolled
...
- `scripts/` — `build.mjs` (esbuild), `vendor-remote-script.mjs`,
  `integration-test.mjs` (live 132-tool test), `list-tools.mjs`, `smoke-send.ts`.
...
- `node scripts/integration-test.mjs` drives all 132 tools (assertions + cleanup).
