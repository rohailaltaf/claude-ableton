...
Walk and load items from Live's instrument and drum browsers. Useful for selecting specific presets and drum kits rather than just loading the default device.
...
| /live/browser/list_samples               | path (optional), offset (optional) | path, offset, total_count, [child_name, ...] | List child names under `app.browser.samples` at the given slash-separated path, with pagination via `offset` for folders that exceed UDP MTU. Reply includes `total_count` so callers can compute when to stop paging. Empty children list (total_count=0) means the path doesn't exist. |
...
| /live/track/load_sample_to_drum_pad      | track_id, device_id, pad_pitch, path |                | Load a sample into a specific Drum Rack pad. `pad_pitch` is the MIDI note that triggers the pad (e.g. 36 = kick). The device at `device_id` must be a Drum Rack. No reply.                  |
...
| /live/clip/automate_device_parameter     | track_id, clip_id, device_id, param_idx, [time, length, value, ...] |                                                                                        | Write step-style automation on a device parameter. Steps tuple is (time, length, value) repeated. Live's AutomationEnvelope only supports steps; approximate ramps with many small adjacent steps. |
| /live/clip/automate_mixer_parameter      | track_id, clip_id, param_name, [time, length, value, ...]           |                                                                                        | Automate a mixer parameter on the clip's track. `param_name` is `volume`, `panning`, or `send_N` (where N is the return index).                          |
| /live/clip/clear_envelopes               | track_id, clip_id                                                   |                                                                                        | Clear all automation envelopes on the clip.                                                                                                              |
