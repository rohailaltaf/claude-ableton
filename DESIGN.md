...
> **Status**: implemented and shipping. Node/TypeScript MCP server, ~107 tools, distributed as a Claude plugin (Code + Desktop) and via `npx` for other MCP clients. This doc captures design decisions; install/usage lives in [README.md](README.md).
...
- **Browser clips (`.alc`) into a Session slot — confirmed ceiling, twice.** This was investigated thoroughly across v0.1.6 and v0.1.8. First pass: walking `app.browser.clips` + `app.browser.load_item(node)` silently no-ops on both MIDI and audio tracks, across five focus strategies (`highlighted_clip_slot`, `selected_track+selected_scene`, `+focus_view("Session")`, `+hotswap_target=slot`, `+schedule_message`). Second pass: exhaustive `dir()` of `Live`, `Live.Application`, `Live.Browser`, `Live.Song`, `Live.Track`, `Live.Clip`, `Live.ClipSlot`, `Live.Device`, `app.view`, `song`, `track`, and every `app.browser.*` child including `user_folders` / `legacy_libraries` / `user_library`. The full inventory of LOM load methods is: `app.browser.load_item` (device-only), `ClipSlot.create_clip` (empty MIDI only), `ClipSlot.create_audio_clip` (audio files only — `load_audio_clip`, below), `ClipSlot.duplicate_clip_to` (copy from existing slot). That's it. **Third confirmation — Ableton's own hardware doesn't do it:** decompiled `Push2/browser_component.pyc` (Python 3.11 bytecode, disassembled with `dis` on a real 3.11 interpreter). Push's browser exposes no "clips" filter type — only `instruments` / `drum_pad` / `audio_effects` / `midi_effects` / `samples` — and `NewTrackBrowserComponent.browse_for_audio_clip` browses **samples**, with `_load_item` calling the same device-only `self._browser.load_item(item)`. So Push loads raw samples (then `create_audio_clip`-equivalent on an audio track), exactly mirroring `load_audio_clip`; it has no secret clip-into-slot LOM call. Reconstruction-from-XML isn't realistic either: MIDI `.alc` decompress to ~228K lines of Live-project XML (full track + instrument rack + clip, hard-coded `/Volumes/data/tmp/trunk/...` pack paths Live resolves via its internal pack registry — reimplementing that is reimplementing Live's set loader). Audio `.alc` are smaller (1–7K lines: `<AudioClip>` + `<SampleRef>` + an `<AudioEffectGroupDevice>` chain), so partial reconstruction is *conceivable* (load the SampleRef via `load_audio_clip`, rebuild the effect rack via `load_audio_effect` + `set_device_parameter`) but lossy (Rack macros, exotic devices, warp markers don't round-trip) and a project in its own right — deliberately shelved.
