...
| `list_samples / load_sample` | shipped | Walks `app.browser.samples` via our fork's `/live/browser/list_samples` and `/live/track/load_sample`. Live wraps loaded samples in Simpler automatically so MIDI plays them transposed. Unlocks real vocal chops / drum hits / texture loops instead of synthesized-from-MIDI sounds. |
