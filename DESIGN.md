...
- **Browser clips (`.alc`) into a Session slot.** Tried in v0.1.6 then rolled back. Walking `app.browser.clips` + `app.browser.load_item(node)` silently no-ops on both MIDI and audio tracks — verified across five focus strategies (`highlighted_clip_slot`, `selected_track+selected_scene`, `+focus_view("Session")`, `+hotswap_target=slot`, `+schedule_message`). Live's UI drag-drop works; `load_item` doesn't bridge for clip-type browser items (it's device-only). A `.alc` is an XML wrapper anyway — to land an actual audio loop from one, parse it for the sample reference and use `load_audio_clip` below.
